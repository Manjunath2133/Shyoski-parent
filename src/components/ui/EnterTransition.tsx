    ...
    const handler = (e: any) => {
      ...
      setTimeout(() => {
        ...
        try {
          ...
          setTimeout(() => {
            const root = document.getElementById('shyoskitalk-root')
            if (root) {
    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-[100] flex items-center justify-center bg-gradient-to-b from-cyan-900/60 to-indigo-900/70 backdrop-blur-md pointer-events-auto"
        >
          ...
          >
            ...
            <p className="text-sm text-white/90 text-center max-w-lg">ShyoskiTalk — initializing offline translation particles. Preparing a private, high-performance conversational bridge...</p>
